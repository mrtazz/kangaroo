 ...
 */
public class MobileMultiTargetDijkstraRouter implements IRouter {
	     ...
	     */
	    private static final Logger LOG = Logger.getLogger(MobileMultiTargetDijkstraRouter.class.getName());
	    ...
	    public Route route(final IDataSet aMap, final Collection<Node> aTargetNodes, final Node aStartingPoint, final IVehicle aSelector) {
	        ...
	        SortedSet<Node> nodesToVisit = new TreeSet<Node>(new MobileNodeDistanceComparator(aMap, aStartingPoint, bestDistancesFrom));
	        Set<Long> nodesToVisit_ = new HashSet<Long>();
	        ...
	        for (Node target : aTargetNodes) {
	            ...
	            for (RoutingStep nextStep  : nextSteps) {
	                ...
	                nodesToVisit_.add(nextStep.getStartNode().getId());
	            }
	        }
	        ...
	        while (!nodesToVisit.isEmpty()) {
	            ...
	            nodesToVisit_.remove(currentNode.getId());
	            ...
	            for (RoutingStep nextStep  : nextSteps) {
	                ...
	                if (!bestDistancesFrom.containsKey(nextNode.getId())
	                    ...
	                    boolean doit = false;
	                    if (nodesToVisit_.contains(nextNode.getId())) {
	                    ...
	                    if (!nodeIDsVisited.contains(nextNode.getId())) {
			            	nodesToVisit.add(nextNode);
			            	nodesToVisit_.add(nextNode.getId());
	                    }
